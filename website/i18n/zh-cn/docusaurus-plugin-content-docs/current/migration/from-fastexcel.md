...
description: 从 cn.idev FastExcel 迁移到 Apache Fesod 的完整指南
keywords: [fesod, 迁移, fastexcel, apache, excel, 升级]
---

# 迁移指南:从 FastExcel 迁移到 Apache Fesod

## 概述

本指南为从 cn.idev FastExcel 迁移到 Apache Fesod 提供全面的迁移路线。Apache Fesod 是该项目的演进版本,现已成为 Apache 软件基金会(孵化中)的一部分,提供同样的高性能 Excel 处理能力,并具有更强的社区支持和长期可持续性。
...
由于高度的 API 兼容性和向后兼容的已废弃别名,从 cn.idev FastExcel 迁移到 Apache Fesod 是一个直接的过程。主要工作涉及更新依赖声明和包导入,几乎不需要或不需要逻辑更改。
